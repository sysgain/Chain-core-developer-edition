...
participantctoken=`sudo docker logs $containerId | grep "^client:" | uniq`
#signer config
response=`sudo docker exec $containerId /usr/bin/chain/corectl config -t $generatornetworktoken $blockchainid http://$generatornodeip:$portnumber`
sudo docker restart $containerId
echo "===========================================Storing participant token to the Azure Key Vault======================"
sudo az keyvault secret set --name $participantclienttokenkeyname --vault-name $keyvaultname --value $participantctoken
